"""
Build nemo-v11-GRPO-on-SFT85.ipynb.

Sophisticated GRPO polish stage tailored for an 85% SFT baseline on
Nemotron-3-Nano-30B-A3B, designed to run on a single RTX 6000 Pro (96 GB)
with vLLM colocated inference.

Pipeline:
  Stage 0:  Triton / ptxas / offline wheel setup (same as v7-5 / v9-1)
  Stage 1:  Load Nemotron base, attach LoRA matching SFT config, warm-start
            LoRA from SFT-85 adapter.
  Stage 2:  (Optional) Micro-SFT pass on 4k Sonnet 4.6 thinking traces to
            graft the high-quality reasoning style before RL.
  Stage 3:  Curriculum filter — keep prompts where SFT pass@1 ∈ [0.25, 0.85].
  Stage 4:  GRPO with vLLM colocate rollouts, DAPO-style asymmetric clip,
            refined multi-component reward, paged 8-bit AdamW.
  Stage 5:  Save adapter + zip submission.
"""
...
NB_PATH = Path(__file__).parent / "nemo-v11-GRPO-on-SFT85.ipynb"
...
cells.append(md("## Reward Functions (de-saturated for 85% baseline)"))
...
cells.append(md("## Stage 4 — GRPO Training"))
